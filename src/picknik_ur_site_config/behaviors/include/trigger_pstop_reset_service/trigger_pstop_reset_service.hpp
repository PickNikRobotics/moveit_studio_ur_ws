 ...
 * @brief TODO(...)
 */
class TriggerPstopResetService : public BT::SyncActionNode
{
public:
  /**
  ...
  /**
   * @brief Implementation of BT::SyncActionNode::tick() for TriggerPstopResetService.
   * @details This function is where the Behavior performs its work when the behavior tree is being run. Since TriggerPstopResetService is derived from BT::SyncActionNode, it is very important that its tick() function always finishes very quickly. If tick() blocks before returning, it will block execution of the entire behavior tree, which may have undesirable consequences for other Behaviors that require a fast update rate to work correctly.
   */
