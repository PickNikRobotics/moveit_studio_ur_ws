...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("setup_mtc_place_pose::SetupMtcPlacePoseBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "SetupMtcPlacePose", BT::NodeConfiguration()));
}
